...
%   * \ZSFref{label}: echter Fernsprung mit Abschnitt und physischer Seite.
%   * \ZSFsectionref{label}: kompakte Zielnummer für lokale Router/Tabellen.
%   * \ZSFseealso{label}{Titel}: benannter lokaler Hinweis.
...
\newcommand{\ZSFref}[1]{%
  ...
  \hyperref[#1]{{\ZSFfontNote\upshape\bfseries\color{\ZSF@refTargetColor}%
    ($\rightarrow$\,\ref*{#1}\enspace\textperiodcentered\enspace S.\,\pageref*{#1})}}%
  \endgroup
}
...
\newcommand{\ZSFseealso}[2]{\enspace{\ZSFfontTable\sffamily\upshape siehe #2~\ZSFsectionref{#1}}}
\makeatother
